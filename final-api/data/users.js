...
let exportedMethods = {
  async getUserByName(userName, missError) {
    const userCollection = await users();
    const user = await userCollection.findOne({ userName });
    if (!user && !missError){ // 内部调用的时候不需要错误处理
  ...
  async signUp(userInfo) {
    const userCollection = await users();
    // try{
    const isUser = await this.getUserByName(userInfo.userName, true)
    if(isUser){
